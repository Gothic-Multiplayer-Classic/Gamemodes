...
local function accountsDb(name)
    return JSON("colony/" .. name .. ".json")
end

local function buildPayload(playerId, state)
    local position = getPlayerPosition(playerId)
    local angle = getPlayerAngle(playerId) or ColonyConfig.spawnAngle
    local hp = getPlayerHealth(playerId) or 0
    local maxHp = getPlayerMaxHealth(playerId) or ColonyConfig.maxHealth
    ...
    return {
        ...
        classId = state.classId or 1,
        moderator = state.moderator and 1 or 0,
...
function ColonyAccounts.exists(name)
...
function ColonyAccounts.save(playerId, state)
...
function ColonyAccounts.load(playerId, state)
    local name = getPlayerName(playerId)
    if not name or name == "" then
        return false
    end
    ...
    state.classId = payload.classId or 1
    state.moderator = payload.moderator == 1
    state.guild = payload.guild or 0
